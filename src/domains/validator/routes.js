...
// Create a 30 Validators
router.post('/create30', async (req, res, next) => {
  try {
    let validators = []; // Array to hold the created validators

    for (let i = 0; i < 30; i++) {
      // Create a new validator document
      const newValidator = new Validator({
        index: i, // Set the index field
        owner: Keypair.generate().publicKey,
        address: Keypair.generate().publicKey,
        stake: 100000
      });
    ...
    // Respond with the created validators
    res.status(201).json(validators);
  } catch (error) {
    next(error);
  }
});
